...
$allowedRoles = ["Master", "Waiter", "Cashier"];
		...
		<div class="wrap">
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				if (mysqli_num_rows($live) > 0) {
					echo "<a href = 'Order_Edit.php?id=" . $row['Table_ID'] . "'><button class = 'tbl_btn' style = 'background-color:#FFA1A1;'>" . $row['Table_ID'] . "<br><i class = 'material-icons'>room_service</i></button></a>";

					$ID = $row['Table_ID'];
					$query = "UPDATE `table_data` SET `Live_status` = 'Seated' WHERE `Table_ID` = '$ID'";
					mysqli_query($connected, $query);
				} else {
					echo "<a href = 'Order_Edit.php?id=" . $row['Table_ID'] . "'><button class = 'tbl_btn'>" . $row['Table_ID'] . "<br><i class = 'material-icons'>room_service</i></button></a>";
				}
			}
			?>
		</div>
